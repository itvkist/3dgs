...
def image_undistortion(project_path):
    result = sp.run(['colmap', 'image_undistorter', \
                    '--image_path', os.path.join(project_path, 'images'), \
                    '--input_path', os.path.join(project_path, 'sparse/1'), \
                    '--output_path', os.path.join(project_path, 'dense/1'), \
                    '--output_type', 'COLMAP', \
                    '--max_image_size', '2000'], capture_output=True, text=True)
    return result
...
if __name__=="__main__":
    project_path = './projects/temple_chen_kieu_indoor'
